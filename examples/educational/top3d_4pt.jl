...
function Bcs(nelx,nely,nelz)
    ...
    jl = [nelz nelz] 
    kl = [nelz/2 nelz/2] 
    loaddof = Vector{Int}(undef,size(il)[2])
    for i in size(il)
    loadnid = kl[i]*(nelx+1)*(nely+1)+il[i]*(nely+1)+(nely+1 - jl[i])
    loaddof[i] = Int.(3*loadnid - 1)
    end
...
function top_opt_3d(nelx,nely,nelz, volfrac,penal, rmin)
    ...
    while change > tolx && loop < maxloop 
        ...
        #Objective function
        c =0
        dc = fill(0,(nely,nelx,nelz))
        for i in 1:size(F)[2]
            Ue = U[:,i]
            ce = reshape(sum((Ue[Int.(edofMat)]*KE).*Ue[Int.(edofMat)],dims=2),(nely,nelx,nelz))
            c = c + sum((Emin .+ xPhs.^penal*(E0-Emin)).*ce)      
           dc = dc -penal*(E0 - Emin)*xPhs.^(penal - 1).*ce
        end
